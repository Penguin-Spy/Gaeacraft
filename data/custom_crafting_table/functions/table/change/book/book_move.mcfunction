...
execute if entity @s[tag=!gaeacraft.cct.cart] run data modify block ~ ~ ~ Items[{Slot:8b}].Count set value 1
execute if entity @s[tag=gaeacraft.cct.cart] run data modify entity @e[type=minecraft:chest_minecart,limit=1,sort=nearest,tag=gaeacraft.cct.chest_minecart] Items[{Slot:8b}].Count set value 1
